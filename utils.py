...
def plot_degradacion_acumulada_dimension(
    ...
    distancias_buffer = list(
        range(
            intervalo_buffer_metros,
            (distancia_paisaje_metros + intervalo_buffer_metros),
            intervalo_buffer_metros,
        )
    )  # Creates [500, 1000, 1500, ..., 5000]
    ...
    max_metros = (
        region - extension
    )  # para que el buffer de 5000 metros nunca se salga de la región de 10000
    desplazamientos_x = []
    desplazamientos_y = []
    np.random.seed(42)
    ...
    for i in range(replicas):
        ...
        predio["geometry"] = predio.translate(
            desplazado_x, desplazado_y
        )  # Mover el predio aleatoriamente
        ...
        for dimension_region in dimensiones_list_region:
            dimension_extension = dimension_region.overlay(
                predio_extension, how="intersection"
            )
            ...
            dimension_region["indicador_parche"] = (
                dimension_region["area_Ha"] * dimension_region["INDICADOR"]
            )
            region_indicador = dimension_region["indicador_parche"].sum()
            region_promedio = round(
                region_indicador / (dimension_region.area.sum() / 10000), 4
            )
            ...
            for buf in distancias_buffer:
                ...
                dimension_buf = dimension_extension.overlay(
                    predio_buf, how="intersection"
                )
                ...
                paisaje_buf["indicador_parche"] = (
                    paisaje_buf["area_Ha"] * paisaje_buf["INDICADOR"]
                )
                #
                ...
                out = round(
                    (
                        ((paisaje_indicador) / (dimension_buf.area.sum() / 10_000))
                        - region_promedio
                    ),
                    4,
                )
                ...
                out = (
                    0 if out > 0 else out
                )  # quedarse solo con los valores negativos (0 si es positivo)
                try:
    ...
    for replica in range(replicas):
        for i, dim in enumerate(DIMENSIONES_DICT.values()):
            for buf_index, buffer_value in enumerate(
                valores_indicador_dimensiones[replica * num_dimensions + i]
            ):
                table_data.append(
                    [dim["label"], distancias_buffer[buf_index], buffer_value]
                )

    # Create the DataFrame
    df = pd.DataFrame(
        table_data, columns=["Dimension", "Buffer_Distance", "Indicator_Value"]
    )
    ...
    percentiles = (
        df_max_buffer.groupby("Dimension")["Indicator_Value"]
        .quantile([percentil_inferior, percentil_superior])
        .unstack()
    )
    ...
    valores_accu_predio_maxbuff = df_max_buffer.groupby("Dimension")[
        "Indicator_Value"
    ].first()
    ...
    ubicacion_lineas = [
        ...
        max_buffer + 30,
        max_buffer + 60,
    ]  # Adjusted for the new max buffer distance
    ...
    for i, dimension in enumerate(percentiles.index):
        ...
        plt.plot(
            df_dim["Buffer_Distance"],
            df_dim["Indicator_Value"],
            label=dimension,
            color=colors[i],
            marker=None,
        )
    ...
    for i, dimension in enumerate(percentiles.index):
        ...
        plt.axvline(
            x=ubicacion_lineas[i],
            ymin=normalized_ymin,
            ymax=normalized_ymax,
            color=colors[i],
            linestyle="--",
            linewidth=2,
            alpha=0.7,
            label=f"Percentiles {percentil_inferior}-{percentil_superior} {dimension}",
        )
...
def plot_valores_conservacion(
    ...
    for dimension_path in dimensiones_list:
        ...
        for buf in distancias_buffer:
            ...
            dimension_buf = dimension.overlay(
                predio_buf, how="intersection"
            )  #  Intersección de la DIMENSION con el BUFFER
    ...
    for i, dim in enumerate(DIMENSIONES_DICT.values()):
        ...
        ax.plot(
            distancias_buffer,
            valores_indicador_dimensiones[i],
            label=dim["label"],
            color=dim["color_plot"],
            linewidth=2,
        )
